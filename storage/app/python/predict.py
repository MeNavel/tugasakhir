...
def index_mask():
    model=loadModel('/XAMPP/htdocs/tugasakhir/storage/app/python/model/mobileNet_Mask.pkl')
...
def index_face():
    nmFile = request.args['file']
    mapClass = ('/XAMPP/htdocs/tugasakhir/storage/app/python/map.npz')
    ...
    kelas=len(mapAngka)
    fold_no=2
    nmModel  = ('/XAMPP/htdocs/tugasakhir/storage/app/python/model/modelCNN.h5')
...
MODEL_PATH = '/XAMPP/htdocs/tugasakhir/storage/app/python/model/model.h5'
...
def generate_frames():
    while True:
        ...
        if not success:
            ...
            color = (255, 0, 0) if label == "shield" else (0, 0, 255)
            if np.argmax(y) == 0:
